  ...
  for (const day of data.days) {
    for (const activity of day.activities) {
      ...
      if (photos.length > 1) {
        ...
        // Ensure photoExamples is an array
        if (!Array.isArray(activity.photoExamples)) {
          activity.photoExamples = [];
        }

        // Check if we already added a Google Maps photo to avoid duplicates if run multiple times
        const hasGooglePhoto = activity.photoExamples.some(p => p.credit === "Google Maps User");
        if (!hasGooglePhoto) {
          activity.photoExamples.push(newExample);
        }
      }
