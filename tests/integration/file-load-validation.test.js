...
describe('File Load — Missing Data Labeling (T010)', () => {
  ...
  it('renders "Not provided" for null websiteUrl', () => {
    window.__toggleExpand('act-1-3');
    const details = document.getElementById('details-act-1-3');
    const notProvided = Array.from(details.querySelectorAll('.not-provided')).map(el => el.textContent);
...
describe('File Load — Activity Detail Field Rendering (T026)', () => {
  it('renders all required detail categories for a fully populated activity', () => {
    ...
    const labels = Array.from(details.querySelectorAll('.detail-label')).map(l => l.textContent);
  ...
  it('renders Google Maps link (FR-012)', () => {
    window.__toggleExpand('act-1-1');
    const link = document.querySelector('#details-act-1-1 a[href*="maps.google"]');
  ...
  it('renders tip list items (FR-014)', () => {
    window.__toggleExpand('act-1-1');
    const tips = document.querySelectorAll('#details-act-1-1 .detail-tips li');
    expect(tips.length).toBeGreaterThanOrEqual(1);
  });

  it('renders review links (FR-016)', () => {
    window.__toggleExpand('act-1-1');
    const reviewLinks = document.querySelectorAll('#details-act-1-1 a[aria-label*="review" i]');
    expect(reviewLinks.length).toBeGreaterThanOrEqual(1);
  });

  it('renders website link (FR-017)', () => {
    window.__toggleExpand('act-1-1');
    const websiteLink = document.querySelector('#details-act-1-1 a[aria-label*="website" i]');
...
describe('File Load — Valid Replacement and Invalid Rollback (T033)', () => {
    ...
    const secondItinerary = {
      ...
      days: [{
        ...
        activities: [{
          ...
          image: { url: 'https://example.com/bigben.jpg', alt: 'Big Ben' },
          location: { lat: 51.5007, lng: -0.1246, mapsUrl: 'https://maps.google.com/?q=Big+Ben' },
          price: 'Free (exterior)',
          tips: ['Best viewed from Westminster Bridge'],
          photoSpotTips: ['Shoot from across the Thames for full tower framing'],
          ratingSummary: '4.5/5',
          reviewLinks: [],
          websiteUrl: null
        }]
  ...
  it('shows validation errors for invalid file', () => {
    ...
    const errorItems = document.querySelectorAll('#validation-error-list li');
  ...
  it('clears validation errors when a valid file is loaded', () => {
    // Load invalid first
    window.__loadItinerary(invalidFixture);
    expect(document.getElementById('validation-errors').classList.contains('visible')).toBe(true);

    // Load valid
    window.__loadItinerary(validFixture);
    expect(document.getElementById('validation-errors').classList.contains('visible')).toBe(false);
  });
