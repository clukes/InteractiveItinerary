...
const VALID_FIXTURE = path.resolve(__dirname, '../fixtures/valid-itinerary.json');
const INVALID_FIXTURE = path.resolve(__dirname, '../fixtures/invalid-itinerary-missing-fields.json');
...
test.describe('Map Markers and Route', () => {
  ...
  test('map has correct number of markers and route lines for Day 1', async ({ page }) => {
  ...
  test('map markers have keyboard access (tabindex and aria-label)', async ({ page }) => {
    const markers = page.locator('.map-marker');
...
test.describe('Activities without map coordinates', () => {
  test('"No map" badge shown for activity lacking lat/lng', async ({ page }) => {
    await page.goto('/');
    // Load the Paris fixture which has Seine River Walk without lat/lng
    const fileInput = page.locator('#file-input');
    await fileInput.setInputFiles(VALID_FIXTURE);

    const seineCard = page.locator('.activity-card', { hasText: 'Seine River Walk' });
...
test.describe('Status Toggle Reset', () => {
  ...
  test('clicking done twice reverts status to pending', async ({ page }) => {
    ...
    await doneBtn.click();
    await expect(page.locator('.activity-card').first()).toHaveAttribute('data-status', 'pending');
  });

  test('clicking skipped twice reverts status to pending', async ({ page }) => {
    const skipBtn = page.locator('.btn-skipped').first();
    await skipBtn.click();
    await expect(page.locator('.activity-card').first()).toHaveAttribute('data-status', 'skipped');
...
test.describe('Activity Ordering', () => {
  test('activities are displayed in correct order on Day 1', async ({ page }) => {
    await page.goto('/');
...
test.describe('Activity Details — Optional Fields', () => {
  test('null price shows "Not provided"', async ({ page }) => {
    ...
    const card = page.locator('.activity-card', { hasText: 'Akihabara Electric Town' });
    await card.locator('.activity-header').click();

    const priceSection = card.locator('.detail-section', { hasText: 'Price' });
  ...
  test('null website shows "Not provided"', async ({ page }) => {
    ...
    const websiteSection = card.locator('.detail-section', { hasText: 'Website' });
  ...
  test('empty reviewLinks shows "No reviews available"', async ({ page }) => {
    ...
    await expect(card.locator('.activity-details')).toContainText('No reviews available');
  });
});
...
test.describe('ARIA Accessibility', () => {
  ...
  test('day tabs have aria-controls pointing to content', async ({ page }) => {
    const tabs = page.locator('[role="tab"]');
    const count = await tabs.count();
    for (let i = 0; i < count; i++) {
      await expect(tabs.nth(i)).toHaveAttribute('aria-controls', 'day-content');
    }
  });
  ...
  test('done/skip buttons have aria-pressed', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      await expect(doneBtns.nth(i)).toHaveAttribute('aria-pressed', 'false');
    }

    const skipBtns = page.locator('.btn-skipped');
    const skipCount = await skipBtns.count();
    for (let i = 0; i < skipCount; i++) {
      await expect(skipBtns.nth(i)).toHaveAttribute('aria-pressed', 'false');
    }
  });

  test('status button aria-label includes activity name', async ({ page }) => {
    const doneBtn = page.locator('.btn-done').first();
    const ariaLabel = await doneBtn.getAttribute('aria-label');
    expect(ariaLabel).toContain('Senso-ji Temple');
  });

  test('map SVG has role="img" and descriptive aria-label', async ({ page }) => {
  ...
  test('validation error area has role="alert"', async ({ page }) => {
    await expect(page.locator('#validation-errors')).toHaveAttribute('role', 'alert');
  });
});
...
test.describe('File Reload State Reset', () => {
  test('loading new file clears previous activity statuses', async ({ page }) => {
    await page.goto('/');
    ...
    for (let i = 0; i < count; i++) {
      await expect(cards.nth(i)).toHaveAttribute('data-status', 'pending');
    }
  });

  test('valid file load hides previous validation errors', async ({ page }) => {
    await page.goto('/');
    ...
    // Now load valid file
    await fileInput.setInputFiles(VALID_FIXTURE);
    await expect(page.locator('#validation-errors')).not.toHaveClass(/visible/);
  });
});
...
test.describe('Visual Verification Screenshots', () => {
  test('Day 1 default view', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/e2e/screenshots/day1-default.png', fullPage: true });
  });

  test('expanded activity details', async ({ page }) => {
    await page.goto('/');
    await page.locator('.activity-header').first().click();
    await page.screenshot({ path: 'tests/e2e/screenshots/activity-expanded.png', fullPage: true });
  });
  ...
  test('Day 2 with done status applied', async ({ page }) => {
    ...
    await page.locator('.btn-done').first().click();
    await page.screenshot({ path: 'tests/e2e/screenshots/day2-with-done.png', fullPage: true });
  });

  test('Day 3 empty state', async ({ page }) => {
    await page.goto('/');
    await page.locator('[role="tab"]').last().click();
    await page.screenshot({ path: 'tests/e2e/screenshots/day3-empty.png', fullPage: true });
  });

  test('Paris fixture loaded', async ({ page }) => {
    await page.goto('/');
    await page.locator('#file-input').setInputFiles(VALID_FIXTURE);
    await page.screenshot({ path: 'tests/e2e/screenshots/paris-loaded.png', fullPage: true });
  });

  test('validation errors after invalid file', async ({ page }) => {
    await page.goto('/');
    await page.locator('#file-input').setInputFiles(INVALID_FIXTURE);
    await page.screenshot({ path: 'tests/e2e/screenshots/validation-errors.png', fullPage: true });
  });
});
