...
test.describe("Mobile Trip Flow", () => {
    ...
    test("status is mutually exclusive — done then skipped", async ({
        page,
    }) => {
        const doneBtn = page.locator(".btn-done").first();
        const skipBtn = page.locator(".btn-skipped").first();
    ...
    test("expanded activity auto-collapses on status change", async ({
        page,
    }) => {
    ...
    test("map marker opens Google Maps URL (T019)", async ({
        page,
        context,
    }) => {
    ...
    test("file upload with invalid file shows errors without replacing content (T034)", async ({
        page,
    }) => {
        ...
        const errorItems = await page
            .locator("#validation-error-list li")
            .all();
